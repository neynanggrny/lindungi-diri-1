<aside class="main-sidebar sidebar-dark-primary elevation-4"> 
    <!-- Brand Logo --> 
    <a href="/administrator" class="brand-link"> 
      <img src="/img/bonbon-collecting-medicines-for-mutual-aid-1.png" alt="AdminLTE Logo" class=" img-circle elevation-3" style="width: 70px;"> 
      <span class="brand-text font-weight-light">Lindungi Diri</span> 
    </a> 
 
    <!-- Sidebar --> 
    <div class="sidebar"> 
      <!-- Sidebar user panel (optional) --> 
      <div class="user-panel mt-3 pb-3 mb-3 d-flex"> 
        <div class="image"> 
          <img src="/img/business-3d-close-up-of-businesswoman-in-blue-suit-with-phone-looking-straight.png" class="img-circle elevation-2" alt="User Image" style="width: 90px;"> 
        </div> 
        <div class="info"> 
          <a href="/" class="d-block">Ferisha Anggraeny</a> 
        </div> 
      </div> 
 
      <!-- Sidebar Menu --> 
      <nav class="mt-2"> 
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false"> 
          <!-- Add icons to the links using the .nav-icon class 
               with font-awesome or any other icon font library --> 
            <li class="nav-item"> 
                <a href="/dashbord" class="nav-link active ">                
                <i class="nav-icon fas fa-tachometer-alt"></i> 
                <p>Dashbord<i class="right fas fa-angle-left"></i></p> 
                </a> 
            </li> 
            <li class="nav-item"> 
                <a href="/catatan-perjalanan" class="nav-link"> 
                <i class="nav-icon fas fa-tachometer-alt"></i> 
                <p>Catatan Perjalanan</p> 
                </a> 
            </li> 
            <li class="nav-item"> 
                <a href="/isi-data" class="nav-link"> 
                <i class="nav-icon fas fa-tachometer-alt"></i> 
                <p>Isi Data</p> 
                </a> 
            </li> 
          </ul> 
          
      </nav> 
      <!-- /.sidebar-menu --> 
    </div> 
    <!-- /.sidebar --> 
  </aside>